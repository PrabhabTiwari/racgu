...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $profile = ['id'=>$id,'name'=>$name,'email'=>$email,'role'=>$role,'roleTitle'=>$title,'districtId'=>trim((string)($_POST['districtId'] ?? '')),'avatar'=>'','phone'=>trim((string)($_POST['phone'] ?? '')),'faculty'=>trim((string)($_POST['faculty'] ?? '')),'bloodGroup'=>trim((string)($_POST['bloodGroup'] ?? '')),'joinedDate'=>date('Y-m-d'),'bio'=>'','badge'=>$title];
